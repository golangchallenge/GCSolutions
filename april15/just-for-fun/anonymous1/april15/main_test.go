...
func TestReadBuffersData(t *testing.T) {
	priv, pub := &[32]byte{'p', 'r', 'i', 'v'}, &[32]byte{'p', 'u', 'b'}
...
func TestWriteDiscardsLengthySlices(t *testing.T) {
	var buf bytes.Buffer
	priv, pub := &[32]byte{}, &[32]byte{}
	w := NewSecureWriter(&buf, priv, pub)
	_, err := w.Write(make([]byte, 0xfff0))
	if err == nil {
		t.Fatal("Write should fail on lengthy writes")
	}
	if err != ErrMessageTooLong {
		t.Fatalf("want %q error, got %q", ErrMessageTooLong, err)
	}
}

func TestReadRandomData(t *testing.T) {
	priv, pub := &[32]byte{'p', 'r', 'i', 'v'}, &[32]byte{'p', 'u', 'b'}
	...
	_, err := io.ReadFull(secureR, buf)

	if err == nil {
		t.Fatal("expected error")
	}
	if err != ErrDecryptFailed {
		t.Fatalf("want %q error, got %q", ErrDecryptFailed, err)
	}
}

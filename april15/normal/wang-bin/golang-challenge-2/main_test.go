...
var largeMsg [MaxMsgLength + 1]byte
...
func TestSecureWriter(t *testing.T) {
	...
	// Make sure message larger than 32kb is not allowed.
	_, err = fmt.Fprintf(secureW, string(largeMsg[:]))
	w.Close()
	...
	if err != ErrMsgTooLong {
		t.Fatalf("Unexpected error. Expect %s, got %s.", err, ErrMsgTooLong)
	}
}

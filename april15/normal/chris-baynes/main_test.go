...
func TestSecureWriterBytesWritten(t *testing.T) {
	priv, pub := &[32]byte{'p', 'r', 'i', 'v'}, &[32]byte{'p', 'u', 'b'}
...
func TestSecureReaderShortReads(t *testing.T) {
	...
	if b := buf.Bytes(); string(buf.Bytes()) != msg {
		t.Fatalf("unexpected bytes received:% x, %s", b, b)
	}
}

...
func TestSmallReadPing(t *testing.T) {
	priv, pub := &[32]byte{'p', 'r', 'i', 'v'}, &[32]byte{'p', 'u', 'b'}
	...
	// Ask for the first 3 bytes of the message
	bufHel := make([]byte, 3)
	nHel, err := secureR.Read(bufHel)
	if nHel != 3 || err != nil {
		t.Fatal(err)
	}
	// Decrypt second part of the message
	bufloWorld := make([]byte, 1024)
	nLoWorld, err := secureR.Read(bufloWorld)
	if err != nil {
		t.Fatal(err)
	}
	var buf []byte
	buf = append(bufHel, bufloWorld[:nLoWorld]...)

	// Make sure we have hello world back
	if res := string(buf); res != "hello world\n" {
		t.Fatalf("Unexpected result: %x != %x", res, "hello world")
	}
}

...
func TestReadWriterLargeMessage(t *testing.T) {
	priv, pub := &[32]byte{'p', 'r', 'i', 'v'}, &[32]byte{'p', 'u', 'b'}

	var message = `AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD`
	...
	// Decrypt message, but use a tiny receive buffer so that we have to read
	// multiple times
	buf := make([]byte, 10)
	...
	// Make sure we have the right thing back
	if decrypted != message {
		t.Fatalf("Unexpected result: %s != %s", decrypted, message)
	}
}

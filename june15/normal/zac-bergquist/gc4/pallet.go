...
// Utilization returns a number between 0 and 1 representing
// how full the pallet is.  A pallet with a utilization of 1
// is perfectly packed.
//
// If p is not properly packed, the utlization calculation would
// be skewed, so this should only be called on pallets that are
// valid.
func (p pallet) Utilization() float64 {
	if err := p.IsValid(); err != nil {
		panic("can't compute utilization for an invalid pallet")
	}
	maxarea := float64(palletWidth * palletLength)
	totalarea := 0.0
	for _, b := range p.boxes {
		totalarea += float64(b.w * b.l)
	}

...
func TestDecodeBadInput(t *testing.T) {
	tests := []struct {
		...
		{"bad_header.splice", errHeaderMissing},
		{"missing_track.splice", io.ErrUnexpectedEOF},
	}

	for _, test := range tests {
		_, err := DecodeFile(path.Join("fixtures", test.path))
		if err != test.err {
...
func BenchmarkDecode(b *testing.B) {
	data, err := ioutil.ReadFile(path.Join("fixtures", "pattern_3.splice"))
	if err != nil {
		panic(err)
	}

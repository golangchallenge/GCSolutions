...
func BenchmarkDecodePattern1(b *testing.B) {
	benchmarkDecode(path.Join("fixtures", "pattern_1.splice"), b)
}

func BenchmarkDecodePattern2(b *testing.B) {
	benchmarkDecode(path.Join("fixtures", "pattern_2.splice"), b)
}

func BenchmarkDecodePattern3(b *testing.B) {
	benchmarkDecode(path.Join("fixtures", "pattern_3.splice"), b)
}

func BenchmarkDecodePattern4(b *testing.B) {
	benchmarkDecode(path.Join("fixtures", "pattern_4.splice"), b)
}

func BenchmarkDecodePattern5(b *testing.B) {
	benchmarkDecode(path.Join("fixtures", "pattern_5.splice"), b)
}

func benchmarkDecode(path string, b *testing.B) {
	// Read the file into memory so that the benchmark doesn't include disk io.
	b.StopTimer()

...
func TestDecodeFileHappyPath(t *testing.T) {
	tData := []struct {
		path   string
		output string
	}{
...
func TestDecodeFileErrors(t *testing.T) {
	tData := []struct {
		...
		{"not_a_real_path",
			"open fixtures/not_a_real_path: no such file or directory",
		},
		{"not_a_splice_pattern",
			"unrecognised file type or format",
		},
		{"incomplete_pattern.splice",
			"fixtures/incomplete_pattern.splice: unable to decode\nerror: unexpected EOF",
		},
	}

	for _, exp := range tData {
		_, err := DecodeFile(path.Join("fixtures", exp.path))
		if err.Error() != exp.errMsg {
			t.Fatalf("%s did not return an error with the expected message.\nGot:\n%s\nExpected:\n%s",
				exp.path, err, exp.errMsg)
		}
	}
}

...
// Test the added functiaonlity to change notes
func TestChangeNotes(t *testing.T) {
	decoded, err := DecodeFile(path.Join("fixtures", "pattern_1.splice"))
	if err != nil {
		t.Fatalf("something went wrong decoding %s - %v", "pattern_1.splice", err)
	}

	decoded.SetNoteForPatchAtBeat(5, 3, true)
	decoded.SetNoteForPatchAtBeat(5, 7, true)
	decoded.SetNoteForPatchAtBeat(5, 15, true)
...
`
	if fmt.Sprint(decoded) != expected {
		t.Logf("decoded:\n%#v\n", fmt.Sprint(decoded))
		t.Logf("expected:\n%#v\n", expected)
		t.Fatalf("Wasn't decoded as expect.\nGot:\n%s\nExpected:\n%s", decoded, expected)
	}
}

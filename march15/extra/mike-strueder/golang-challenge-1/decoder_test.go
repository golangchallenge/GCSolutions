...
func TestMoreCowbell(t *testing.T) {
...
`
	...
	steps := []bool{true, false, false, false, true, false, true, false, true, false, false, false, true, false, true, false}
	err = pattern.SetTrackByName("cowbell", steps)
	if err != nil {
		t.Error(err)
	}

	if pattern.String() != expected {
		t.Errorf("Expected:\n%sActual:\n%s", expected, pattern.String())
	}
